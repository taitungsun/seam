...
**Special Education Administration & Management (SEAM) · v1.0**
...
| 模組 | 說明 |
|------|------|
| 📋 學生資料 | 建立完整學生名冊、特教類別、服務項目；支援科別篩選、學籍異動（含歷史紀錄）、去識別化 PDF 匯出 |
| 🗓️ 分組與課表 | 課程分組（含學生名單）與教師課表分開管理，節次時間預設帶入，支援 PDF 匯出 |
| 📊 成績管理 | 依科目建立成績表，自動計算日常考查 40% 與定期評量 60%，與學生名冊連動同步 |
| 💰 經費管理 | 多年度、多計畫經費追蹤，自動計算執行率與餘額，支援應結報日期提醒 |
| ☑️ 特推會審查 | 自動彙整各生特殊教育需求資料，產生對應教育部格式的審查表，一鍵匯出 PDF |
| 🔔 提醒事項 | 整合自訂提醒與經費結報日期，支援 `.ics` 行事曆匯入／匯出（可同步至 Google 日曆） |
| ⚙️ 系統設定 | 設定學校名稱、班型、科系清單，套用至所有頁面標題與匯出報表 |
...
至 [Releases](../../releases) 頁面下載最新版 `SEAM_v1.0.html`，或直接 Clone 本儲存庫。

**2. 開啟系統**

以瀏覽器（Chrome、Edge、Firefox、Safari）開啟 `SEAM_v1.0.html`，不需要任何安裝步驟。
...
- **PDF**：透過瀏覽器列印功能（Ctrl+P / Cmd+P）產生，可選擇「另存為 PDF」。適用於學生去識別化名冊、成績表、特推會審查表、教師課表。
- **CSV**：學生名冊可匯出 CSV 供 Excel 開啟進行進一步處理。
- **ICS**：提醒事項與經費結報日期可匯出為 `.ics` 行事曆檔，匯入 Google 日曆或手機行事曆後即可收到提醒通知。同樣支援從現有 `.ics` 檔匯入事件。
...
本系統設計適用對象為台灣高中職階段特殊教育教師，主要功能對應以下實務需求：

- 特殊教育推行委員會（特推會）審查資料製作
- 普通班身心障礙生輔導經費結報管理
- 資源班課程分組與教師排課
- IEP 相關學生資料管理與歷屆追蹤
...
本系統採 [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/) 授權。您可自由使用、修改與散布，惟需保留原始作者署名。
